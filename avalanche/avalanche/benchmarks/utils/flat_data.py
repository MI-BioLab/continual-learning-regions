    ...
    *Notes for subclassing*
    ...
    def _remove_unused_elements(self):
        ...
        for dataset_idx, dataset in enumerate(self._datasets):
            unused_indices[dataset_idx] = set(range(len(dataset)))

        # Shallow copy of datasets
        self._datasets = [copy.copy(dataset) for dataset in self._datasets]

        for i in range(len(self)):
            dataset_idx, sample_idx = self._get_idx(i)
            unused_indices[dataset_idx].remove(sample_idx)

        for dataset_idx, dataset in enumerate(self._datasets):
            for sample_idx in unused_indices[dataset_idx]:
                dataset[sample_idx] = None
                removed_count += 1
