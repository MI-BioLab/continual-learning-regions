"""
This is a customized version of Avalanche accuracy metric to enable logging
of accuracies using the experience ID as the X axis instead of the train iteration.
"""
...
from plugins import GenericPluginMetric

"""
Modified version of the Accuracy class of Avalanche to deal with the top-k accuracy when n_classes < k.
"""
...
class AccuracyPluginMetric(GenericPluginMetric[float]):
    ...
    def __init__(self, reset_at, emit_at, mode, experience_granularity=False):
        self._accuracy = Accuracy()
        super(AccuracyPluginMetric, self).__init__(
            self._accuracy, reset_at=reset_at, emit_at=emit_at, mode=mode, experience_granularity=experience_granularity
        )
...
class ExperienceAccuracy(AccuracyPluginMetric):
    ...
    def __init__(self, experience_granularity=False):
        """
        Creates an instance of ExperienceAccuracy metric
        """
        super(ExperienceAccuracy, self).__init__(
            reset_at="experience", emit_at="experience", mode="eval", experience_granularity=experience_granularity
        )
...
class StreamAccuracy(AccuracyPluginMetric):
    ...
    def __init__(self, experience_granularity=False):
        ...
        super(StreamAccuracy, self).__init__(
            reset_at="stream", emit_at="stream", mode="eval", experience_granularity=experience_granularity
        )
...
def custom_accuracy_metrics(
    *,
    ...
    if experience:
        metrics.append(ExperienceAccuracy(experience_granularity=experience_granularity))

    if stream:
        metrics.append(StreamAccuracy(experience_granularity=experience_granularity))

    if trained_experience:
        metrics.append(TrainedExperienceAccuracy(experience_granularity=experience_granularity))
